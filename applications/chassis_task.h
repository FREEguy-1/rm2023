...
//鼠标左右移动控制底盘旋转时，鼠标移动距离转化为移动速度的比例
...
typedef struct
{
  ...
  pid_type_def pc_ore_control_pid;              //follow angle PID.底盘跟随pc（金矿）
	pid_type_def pc_servo_angle_pid;             //底盘跟随pc（障碍块角度）
	pid_type_def pc_servo_vy_pid;                 //底盘跟随pc（障碍块左右）
